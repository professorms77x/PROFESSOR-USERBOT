...
@Client.on_message(filters.user(SUDO_USERS) & filters.command(["song"], ["."]))
async def song(client: Client, message: Message):
    input_str = get_text(message)
    rep = await edit_or_reply(message, "ᴘʀᴏᴄᴇꜱꜱɪɴɢ...")
    if not input_str:
        await rep.edit(
            "ɢɪᴠᴇ ᴍᴇ ᴀ ᴠᴀʟɪᴅ ɪɴᴘᴜᴛ...."
        )
        return
    await rep.edit(f"ꜱᴇᴀʀᴄʜɪɴɢ {input_str}")
    search = SearchVideos(str(input_str), offset=1, mode="dict", max_results=1)
    rt = search.result()
    result_s = rt["search_result"]
    url = result_s[0]["link"]
    ...
    try:
        ...
        await rep.edit(f"**ᴇʀʀᴏʀ :** `{str(e)}`")
        return
    time.time()
    file_path = f"{ytdl_data['id']}.mp3"
    capy = f"**ꜱᴏɴɢ ɴᴀᴍᴇ** `{vid_title}` \n**ʀᴇQᴜᴇꜱᴛᴇᴅ ꜰᴏʀ** `{input_str}` \n**ᴄʜᴀɴɴᴇʟ** `{uploade_r}` \n**Link ➠** `{url}`"
